...
A_var* A_SimpleVar(A_pos pos, S_symbol sym) {
    return new A_var(pos, A_simpleVar, sym);
}

A_var* A_FieldVar(A_pos pos, A_var var, S_symbol sym) {
    return new A_var(pos, A_fieldVar, var, sym);
}

A_var* A_SubscriptVar(A_pos pos, A_var var, A_exp exp) {
    return new A_var(pos, A_subscriptVar, var, exp)
}

A_exp A_VarExp(A_pos pos, A_var var) {
    return new A_exp(pos, A_varExp, var);
}

A_exp A_NilExp(A_pos pos) {
    return new A_exp(pos, A_nilExp);
}

A_exp A_IntExp(A_pos pos, int i) {
    return new A_exp(pos, A_intExp, i);
}

A_exp A_StringExp(A_pos pos, string s) {
   return new A_exp(pos, A_stringExp s);
}

A_exp A_CallExp(A_pos pos, S_symbol func, A_expList args) {
    return new A_exp(pos, A_callExp, func, args);
}

A_exp A_OpExp(A_pos pos, A_oper oper, A_exp left, A_exp right) {
    return new A_exp(pos, A_opExp, oper, left, right);
}

A_exp A_RecordExp(A_pos pos, S_symbol typ, A_efieldList fields) {
    return new A_exp(pos, A_recordExp, typ, fields);
}
